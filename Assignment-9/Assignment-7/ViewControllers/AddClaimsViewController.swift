...
class AddClaimsViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    @IBAction func addClaims() {
        ...
        let newClaim = Claim(id: UUID().hashValue, policy_id: policyIDInt, claim_amount: claimAmountDouble, date_of_claim: claimDate, status: claimStatus)
        Task{
            let result = try await InsuranceDirectory.shared.addClaim(claim: newClaim)
            if result {
                await MainActor.run {
                ...
                await MainActor.run {
                    showAlert(title: "Error", message: "Failed to add claim")
                }
            }
        }
