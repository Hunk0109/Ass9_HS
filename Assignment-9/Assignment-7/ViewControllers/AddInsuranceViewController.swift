...
class AddInsuranceViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    @IBAction func addInsurance() {
        ...
        let newInsurance = Insurance(
            ...
            policy_type: policyTypeText,
            premium_amount: premiumDouble,
            start_date: startDatePicked,
            end_date: endDatePicked
        )
        
        
        Task{
            do{
                let n = await InsuranceDirectory.shared.addInsurance(i: newInsurance)
                if n {
                    await MainActor.run {
                    ...
                    await MainActor.run {
                        showAlert(title: "Error", message: "An error occurred while adding insurance.")
                    }
                }
            }
        }
