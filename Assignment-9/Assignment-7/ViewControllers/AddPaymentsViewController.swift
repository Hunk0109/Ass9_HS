...
class AddPaymentsViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    @IBAction func addPayment(_ sender: UIButton) {
        ...
        let newPayment = Payment(id: UUID().hashValue, policy_id: policyIdInt, payment_amount: paymentAmountDouble, payment_date: payDate, payment_method: paymentMethodText, status: paymentStatusText)
        Task{
            let result = try await InsuranceDirectory.shared.addPayment(payment: newPayment)
            if result{
                await MainActor.run {
