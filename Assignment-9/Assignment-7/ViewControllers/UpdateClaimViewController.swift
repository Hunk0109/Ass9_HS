...
class UpdateClaimViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    @IBAction func updateClaim(_ sender: UIButton) {
        ...
        Task{
            let c = await InsuranceDirectory.shared.updateClaim(claim: claim!)
            if c {
                await MainActor.run {
                    ...
                    await MainActor.run {
                        ...
                        showAlert(title: "Error", message: "Error updating claim")
                    }
                }
        }
    ...
    @IBAction func deleteClaim(_ sender: UIButton) {
        ...
        Task{
            let result = try await InsuranceDirectory.shared.deleteClaim(clm: customClaim)
            
            if result{
                await MainActor.run {
                    showAlert(title: "Success", message: "Claim deleted successfully",handler: {_ in self.navigationController?.popViewController(animated: true)
                    })
                    
                }
            } else {
                await MainActor.run {
                    showAlert(title: "Error", message: "Error deleting claim")
                }
            }
        }
    }
