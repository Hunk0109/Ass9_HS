...
class UpdateInsuranceViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    @IBAction func updateInsurance(){
        ...
        let updatedInsurance = Insurance(id: insurance.id, customer_id: insurance.customer_id, policy_type: policyTypeText, premium_amount: premiumDouble, start_date: insurance.start_date, end_date:endDatePicked )
        
        Task {
            let i = await InsuranceDirectory.shared.updateInsurance(update: updatedInsurance)
            if i {
                await MainActor.run {
    ...
    @IBAction func deleteInsurance() {
        ...
        Task{
            let result = await InsuranceDirectory.shared.deleteInsurance(id: insurance.id)
            
            if result{
                await MainActor.run {
                    showAlert(title: "Success", message: "Insurance deleted.", handler: {_ in self.navigationController?.popViewController(animated: true)
                    })
                }
            } else {
                await MainActor.run {
                    showAlert(title: "Error", message: "Insurance could not be deleted.")
                }
            }
        }
