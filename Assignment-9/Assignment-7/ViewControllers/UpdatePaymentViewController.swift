...
class UpdatePaymentViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    @IBAction func updatePayment(_ sender: UIButton) {
        ...
        if let payment{
            ...
            Task{
                let result = try await InsuranceDirectory.shared.updatePayment(payment: payment)
                if result{
                    await MainActor.run {
                    ...
                    await MainActor.run {
                        showAlert(title: "Error", message: "Unable to update payment.")
                    }
                }
            }
        }
        ...
        if let payment{
            if payment.status == "Processed"{
                ...
                if let customPayment{
                    Task{
                        let result = try await InsuranceDirectory.shared.deletePayment(pay: customPayment)
                        if result{
                            await MainActor.run {
                                showAlert(title: "Success", message: "Payment deleted successfully.", handler: {_ in
                                    self.navigationController?.popViewController(animated: true)
                                })
                            }
                        } else {
                            await MainActor.run {
                                showAlert(title: "Error", message: "Error deleting payment.")
                            }
                        }
